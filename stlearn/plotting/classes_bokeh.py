...
class BokehClusterPlot(Spatial):
    def __init__(self,
        ...
        self.checkbox_group = CheckboxGroup(labels=["Show spatial trajectories"], active=[])
    ...
    def make_fig(self):
        ...
        for i in self.list_cluster.active:
            command.append(self.use_label + ' == "' + self.adata[0].obs[self.use_label].cat.categories[i] + '"')
        ...
        if 0 in self.checkbox_group.active:
            ...
            for edges in G.edges:
                fig.add_layout(Arrow(end=VeeHead(fill_color="#f4efd3",line_alpha=0,line_width=5),
                                   x_start=centroid_dict[edges[0]][0],
                                     y_start=centroid_dict[edges[0]][1], 
                                     x_end=centroid_dict[edges[1]][0], 
                                     y_end=centroid_dict[edges[1]][1],line_color="#f4efd3",
                                    line_width=5))
                set_x.append(centroid_dict[edges[0]][0])
                set_y.append(centroid_dict[edges[0]][1])
                
            fig.circle(x=set_x, y=set_y, radius=50,
                     color="#f4efd3", fill_alpha=0.3, line_alpha=0)
...
class BokehCciPlot(Spatial):
    ...
    def _get_het(self,het):
        ...
        if het not in self.adata[0].obsm:
            raise ValueError(
                het + " is not exist in the data, please try another het"
            )
