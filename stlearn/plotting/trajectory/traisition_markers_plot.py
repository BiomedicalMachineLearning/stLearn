...
def traisition_markers_plot(adata,top_genes=10,trajectory=None):
    ...
    for i,rect in enumerate(rects):
        ...
        axes[1].text(rect.get_x()+rect.get_width()+0.01, rect.get_y() + rect.get_height()/2.0, \
                gene_name,**alignment,size=6)
        axes[1].text(rect.get_x()+0.01, rect.get_y()+rect.get_height()/2.0,
                p_value,
    ...
    for i,rect in enumerate(rects):
        ...
        axes[0].text(rect.get_x()+rect.get_width()-0.01, rect.get_y() + rect.get_height()/2.0, \
                gene_name,**alignment,size=6)
        axes[0].text(rect.get_x()-0.01, rect.get_y()+rect.get_height()/2.0,
                p_value,
               color='w',**alignment,size=6)
