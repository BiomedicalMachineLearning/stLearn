...
class SpatialBasePlot(Spatial):
    def __init__(self,
        ...
        **kwds
        ...
        assert cmap in cmap_available, "cmap should be one of these colors: " + str(cmap_available)
        self.cmap = cmap
    ...
    def _select_clusters(self):
        ...
        if self.list_clusters is not None:
            self.query_adata = self.query_adata[
                self.query_adata.obs.query(create_query(self.list_clusters, self.use_label)).index
            ].copy()
        else:
...
class GenePlot(SpatialBasePlot):
    def __init__(self,
        ...
        **kwargs
        ...
        assert method in method_available, "Please choose available method in: " + str(method_available)
        self.method = method
    ...
    def _get_gene_expression(self):
        ...
        if len(self.gene_symbols) == 0:
            ...
            if self.gene_symbols[0] not in self.query_adata.var_names:
                raise ValueError(
                    self.gene_symbols[0] + " is not exist in the data, please try another gene"
                )
    ...
    def _plot_contour(self,gene_values: pd.Series):
        ...
        cs = plt.contourf(xi,yi,zi,range(0,int( np.nanmax(zi))+self.step_size,self.step_size),
                          cmap=plt.get_cmap(self.cmap),alpha=self.cell_alpha)
        return cs
...
class ClusterPlot(SpatialBasePlot):
    ...
    def _plot_clusters(self):
        ...
        for i, cluster in enumerate(self.query_adata.obs.groupby(self.use_label)):
            # Plot scatter plot based on pixel of spots
            subset_spatial = self.query_adata.obsm["spatial"][check_sublist(list(self.query_adata.obs.index),
                                                                            list(cluster[1].index))]
    ...
    def _add_cluster_labels(self):
        ...
        for i, label in enumerate(self.list_clusters):
            ...
            label_index = list(self.query_adata.obs[self.query_adata.obs[self.use_label]==str(label)].index)
            subset_spatial = self.query_adata.obsm["spatial"][check_sublist(list(self.query_adata.obs.index),
                                                                            label_index)]
    ...
    def _add_sub_clusters(self):
        ...
        for i, label in enumerate(self.list_clusters):
            ...
            if (
                len(
                    self.query_adata.obs[self.query_adata.obs[self.use_label] == str(label)][
                        "sub_cluster_labels"
                    ].unique()
                )
                < 2
            ...
            for j, label in enumerate(classes):
                if (
                    len(self.query_adata.obs[self.query_adata.obs["sub_cluster_labels"] == label])
            ...
            for x, y in centroid_dict.items():
                ...
                if x in get_node(list_cluster, self.adata[0].uns["split_node"]):
                    a.text(
                        ...
                        bbox=dict(
                            facecolor=cmap(
                                int(get_cluster(str(x), self.adata[0].uns["split_node"]))
                                / (len(used_colors) - 1)
                            ),
                            boxstyle="circle",
                            alpha=1,
