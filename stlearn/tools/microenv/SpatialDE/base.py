...
def SpatialDE(
    ...
    sample_resid_expr = resid_expr.sample(
        n=len(adata.obsm["filtered_counts"].columns), axis=1, random_state=1
    )
    ...
    adata.uns["SpatialDE_top_genes"] = (
        mll_results.sort_values("qval").head(top_genes)["g"].tolist()
    )
