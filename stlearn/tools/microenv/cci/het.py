...
def count(
    ...
    if distance != 0:
        ...
        for spot in adata.obs_names:
            ...
            counts_ct.loc[spot] = ((adata.uns[use_clustering].loc[neighbours] > 0.2).sum() > 0).sum()      
...
def create_grids(
    ...
    for n in range(num_row * num_col):
        ...
        nb_matrix = [[a[i][j] if  0 <= i < a.shape[0] and 0 <= j < a.shape[1] else -1 for j in range(col-radius, col+1+radius)] for i in range(row-radius, row+1+radius)]
        for item in nb_matrix:
            neighbour = chain(neighbour, item)
        neighbour = list(set(list(neighbour)))
        neighbours.append([grid for grid in neighbour if not (grid == n and radius > 0) and grid != -1])
...
def count_grid(
    ...
    for n, grid in enumerate(grids):
        spots = coor[(coor['imagecol'] > grid[0]) & (coor['imagecol'] < grid[0] + width) \
                    & (coor['imagerow'] < grid[1]) & (coor['imagerow'] > grid[1] - height)]
        counts.loc[n] = (adata.uns[use_clustering].loc[spots.index] > 0.2).sum().sum()
    adata.uns[use_het] = (counts / counts.max())['CT']

    print("Counts for cluster (cell type) diversity stored into data.uns[\'" + use_het + "\']")
