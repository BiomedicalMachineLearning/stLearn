...
def get_hotspots(adata: AnnData, lr_scores: np.ndarray, lrs: np.array,
                 eps: float, quantile=0.05, verbose=True,
                 plot_diagnostics: bool=False, show_plot: bool=False):
    ...
    lr_summary, lr_hot_scores = hotspot_core(lr_scores, lrs, coors, eps,
                                             quantile, plot_diagnostics, adata)
    ...
    dist_cutoff = np.quantile(clusterer.distances_, .98)
    clusterer = AgglomerativeClustering(affinity='euclidean', linkage='ward',
    ...
    lr_summary = pd.DataFrame(lr_summary, index=lrs,
                              columns=['spot_counts', 'cutoff',
                                       'hotspot_counts', 'lr_cluster'])
    ...
    if verbose:
        ...
        print(f"\tMatrix of lr scores in same order as the summary in adata.obsm['lr_scores'].")
        print(f"\tMatrix of the hotspot scores in adata.obsm['lr_hot_scores'].")
        print(f"\tMatrix of the mean LR cluster scores in adata.obsm['cluster_scores'].")

def hotspot_core(lr_scores, lrs, coors, eps, quantile, plot_diagnostics=False,
                 adata=None, verbose=True, max_score=False):
    ...
    if plot_diagnostics:
        ...
        quant_lrs = np.array([lrs[lr_mean_scores == quant]
                              for quant in lr_quant_values])
    ...
    with tqdm(total=len(lrs),
              ...
              bar_format="{l_bar}{bar}", disable=verbose==False,
              ) as pbar:
        for i, lr_ in enumerate(lrs):
            ...
            for cutoff in cutoffs:
                ...
                score = len(np.unique(clusters)) * \
                        (np.mean(lr_score_[spot_bool])) ** 2
            ...
            if plot_diagnostics and lr_ in quant_lrs and \
                    type(adata)!=type(None):
                add_diagnostic_plots(adata, i, lr_, quant_lrs, lr_quantiles,
                                     lr_scores, lr_hot_scores,
                                     axes, cutoffs, cutoff_scores,
                                     best_cutoff, )
...
def add_diagnostic_plots(adata, i, lr_, quant_lrs, lr_quantiles,
                         lr_scores, lr_hot_scores,
                         axes, cutoffs, n_clusters, best_cutoff,):
    ...
    scores_ = lr_scores[i, :]
    sb.distplot(scores_[scores_ > 0], ax=axes[q_i][1],
                hist=True, kde=False, color='red', norm_hist=True)
    v_height = .5
    axes[q_i][1].vlines(best_cutoff, 0, v_height)
    axes[q_i][1].text(best_cutoff, v_height, str(round(best_cutoff, 2)))
    axes[q_i][1].set_title(
        f'Distrib {round(lr_quantiles[q_i], 2)}({lr_})')

    # Showing before & after filtering spots #
    adata.obsm['lr_scores'] = scores_
    het_plot(adata, use_het='lr_scores', ax=axes[q_i][2],
             show_color_bar=False,
             )
    axes[q_i][2].set_title('scores')

    adata.obsm['lr_scores'] = lr_hot_scores[i, :]
    het_plot(adata, use_het='lr_scores', ax=axes[q_i][3],
             show_color_bar=False,
             )
