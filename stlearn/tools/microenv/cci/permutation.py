...
def get_rand_pairs(
    ...
    lr (int):               The lr pair string to get equivalent random pairs
                            for (e.g. 'L_R')
    Returns
    -------
    pairs (list)            List of random gene pairs with equivalent mean expression
                            (e.g. ['L_R'])
    """
    ...
    means_ordered, genes_ordered = get_ordered(adata, genes)
    if im is None:  # Single background per lr pair mode
        ligand, receptor = lrs[0].split("_")
        im = get_median_index(ligand, receptor, means_ordered.values, genes_ordered)
...
def get_median_index(ligand, receptor, means_ordered, genes_ordered):
    ...
    i1 = np.where(genes_ordered == ligand)[0][0]
    i2 = np.where(genes_ordered == receptor)[0][0]
