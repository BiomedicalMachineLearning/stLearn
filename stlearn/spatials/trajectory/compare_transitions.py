...
def compare_transitions(adata,trajectories):
    ...
    pos_1 = list(adata.uns[trajectories[0]][adata.uns[trajectories[0]]["score"] >=0]["gene"])
    pos_2 = list(adata.uns[trajectories[1]][adata.uns[trajectories[1]]["score"] >=0]["gene"])
    compare_pos_1 = np.setdiff1d(pos_1,pos_2,assume_unique=True)
    compare_pos_2 = np.setdiff1d(pos_2,pos_1,assume_unique=True)
    
    neg_1 = list(adata.uns[trajectories[0]][adata.uns[trajectories[0]]["score"] <0]["gene"])
    neg_2 = list(adata.uns[trajectories[1]][adata.uns[trajectories[1]]["score"] <0]["gene"])
    ...
    print("The result of comparison between " + trajectories[0] + " and " + trajectories[1] + \
