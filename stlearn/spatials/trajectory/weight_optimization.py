...
def weight_optimizing_global(adata,use_label=None,list_cluster=None,step=0.01,k=10):
	...
	with tqdm(total=int(1/step + 1), desc="Screening", bar_format="{l_bar}{bar} [ time left: {remaining} ]") as pbar:
		for i in range(0,int(1/step + 1)):

		    Gs.append(nx.adjacency_matrix(global_level(adata,use_label=use_label,list_cluster=list_cluster,w=round(j,2),return_graph=True,verbose=False)))
	...
	indx = []
	w = 0
	with tqdm(total=int(1/step-1), desc="Calculating", bar_format="{l_bar}{bar} [ time left: {remaining} ]") as pbar:
