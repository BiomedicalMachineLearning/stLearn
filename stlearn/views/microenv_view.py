...
class ChangeImage:
    def __init__(self, root, adata, use_data):
        ...
        self.bt_plot = tk.Button(
            ...
            text="Plot!",
            command=lambda: [
                plot(self.adata, use_data, self.image_num + 1),
                root.destroy(),
            ],
            bg="#007bff",
    ...
    def select_images(self, *args):
        if (int(self.v.get()) > len(self.photos)) or (int(self.v.get()) <= 0):
            messagebox.showerror(
                "Error",
                "Please choose available factor! (0 < factor <= "
                + str(len(self.photos))
                + ")",
            )
            return None
    ...
    def save(self):
        ...
        img = Image.fromarray(
            self.adata.uns["plots"][self.use_data]["factor_" + str(self.image_num + 1)]
        )
...
def plot(
    ...
    if (factor > len(adata.uns["plots"][use_data])) or (factor <= 0):
        raise ValueError(
            "Error",
            "Please choose available factor! (0 < factor <= "
            + str(len(adata.uns["plots"][use_data]))
            + ")",
        )
    else:
