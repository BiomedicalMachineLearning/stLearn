...
class TestCCI(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        * A is the middle spot, B/C/D/E/F/G are the neighbouring spots clock-
        ...
        for i, bc in enumerate(spot_bcs):
            ...
            neigh_indices = np.array([np.where(spot_bcs==n_bc)[0][0]
                                                         for n_bc in neigh_bcs])
    ...
    def test_edge_retrieval_basic(self):
        ...
        het_hs.get_between_spot_edge_array(edge_list,
        ...
        for bc in self.neigh_dict:
            ...
            for neigh_bc in neigh_bcs:
                if (bc, neigh_bc) not in all_edges and \
                    (neigh_bc, bc) not in all_edges:
                    all_edges.append((bc, neigh_bc))
        n_edges = len( all_edges )

        self.assertEqual(len(edge_list), n_edges)
        self.assertTrue(np.all([edge in all_edges or edge[::-1] in all_edges
        ...
        edge_list = het_hs.init_edge_list(neighbourhood_bcs)
        invalid_neighs = ['B', 'E']
        neigh_bool = np.array([bc not in invalid_neighs
                                                     for bc in self.neigh_dict])
        het_hs.get_between_spot_edge_array(edge_list,
                                       neighbourhood_bcs, neighbourhood_indices,
                                                          neigh_bool, cell_data)
        ...
        sub_edges1 = [edge for edge in all_edges if edge not in
                      [('B', 'C'), ('C', 'B')]]
        n_subedges = len(sub_edges1)

        self.assertEqual(len(edge_list), n_subedges)
        self.assertTrue(np.all([edge in sub_edges1 or edge[::-1] in sub_edges1
                                for edge in edge_list]))
        ...
        edge_list = het_hs.init_edge_list(neighbourhood_bcs)
        het_hs.get_between_spot_edge_array(edge_list, neigh_bcs, neigh_indices,
                                                          neigh_bool, cell_data)
        edge_list = edge_list[1:] #Remove initialisation edge
        ...
        self.assertTrue(np.all([edge in sub_edges2 or edge[::-1] in sub_edges2
                                for edge in edge_list]))
        ...
        self.assertTrue(np.all([edge in sub_edges3 or edge[::-1] in sub_edges3
                                for edge in edge_list]))
    ...
    def test_get_interactions(self):
        ...
        interaction_edges = het.get_interactions(cell_data,
                             self.neighbourhood_bcs, self.neighbourhood_indices,
                                           all_set, sig_bool, l_bool, r_bool, 0)
        ...
        for i in range(len(expected_edges)):
            ...
            match_bool = [edge in observed_edgesi or
                          edge[::-1] in observed_edgesi
